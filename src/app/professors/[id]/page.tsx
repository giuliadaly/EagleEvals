    ...
    <>
      ...
      <main className="flex-1">
        <section className="border-b border-[var(--line)] bg-white py-8 sm:py-12">
          <div className="page-shell">
            ...
            <div className="mt-8 flex flex-wrap gap-2 text-xs font-semibold text-[var(--muted)]">
              ...
              {professor.commentCount ? <span className="rounded-full bg-[var(--wash)] px-3 py-2">{formatCount(professor.commentCount)} written review{professor.commentCount === 1 ? "" : "s"}</span> : null}
            ...
            <div className="mt-6 flex flex-wrap gap-3"><Link className="button-primary" href={`/evaluations?q=${encodeURIComponent(professor.name)}`}>Browse all evaluations</Link><Link className="button-gold" href={`/review?professor=${professor.id}`}>Write anonymous review</Link></div>
          </div>
        </section>
        ...
        <div className="page-shell">
          ...
          {comments.length ? <section className="detail-section"><SectionHeading eyebrow="Student perspective" title="Written reviews" description="Recovered and new anonymous comments are shown as written and may reflect different course formats." href={`/comments?q=${encodeURIComponent(professor.name)}`} linkLabel="Browse all matching comments" /><div className="grid gap-4 lg:grid-cols-2">{comments.map((comment) => <CommentCard key={comment.id} comment={comment} context="professor" />)}</div></section> : null}
