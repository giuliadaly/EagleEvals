...
export const metadata: Metadata = { title: "Professors", description: "Search and filter restored historical evaluations for Boston College professors." };
...
export default async function ProfessorsPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string; sort?: string; min?: string }> }) {
  ...
  const sort = ["rating", "course", "name"].includes(params.sort ?? "") ? params.sort! : "evidence";
      ...
      <main className="flex-1">
        <section className="border-b border-[var(--line-strong)] bg-[var(--paper-raised)] py-10 sm:py-14">
          <div className="page-shell">
            ...
            <p className="mt-3 text-sm text-[var(--muted)]">{formatCount(data.total)} matching professors · keep ratings in context with evidence volume</p>
            <form role="search" className="mt-8 grid gap-3 rounded-[.75rem] border border-[var(--line-strong)] bg-[var(--paper-ledger)] p-4 sm:grid-cols-[minmax(0,1fr)_13rem_11rem_auto] sm:items-end">
              ...
              <label><span className="form-label">Order by</span><select name="sort" defaultValue={sort} className="form-control"><option value="evidence">Most evaluations</option><option value="rating">Highest instructor rating</option><option value="course">Highest course rating</option><option value="name">Name A–Z</option></select></label>
